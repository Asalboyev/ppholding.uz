...
@section('content')
    ...
    <div class="headcrumbs d-flex mb-3">
        <a href="{{ route('products.index') }}" class="d-flex text-uppercase me-2" style="opacity:25%">products</a> >> <a class="d-flex text-uppercase ms-2">edit</a>
    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-lg-6 col-sm-6">
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <input type="text" class="form-control" name="title[{{ $language->small }}]" placeholder="title..." value="{{ old('title.'.$language->small) ? old('title.'.$language->small) : $product->title[$language->small] }}">
                                            </div>
                                            <div class="my-4">
                                                <label for="textarea">Description</label>
                                                <textarea class="form-control ckeditor" placeholder="Enter your description..." id="textarea" rows="4" name="desc[{{ $language->small }}]">{{ old('desc.'.$language->small) ? old('desc.'.$language->small) : $product->desc[$language->small] }}</textarea>
                                            </div>
                                            <!-- End of Form -->
                                        </div>
                                    </div>
                            ...
                            <div class="col-6">
                                <div class="mb-4">
                                    ...
                                    <select class="form-select" id="catalog" name="catalog">
                                        ...
                                        @foreach($catalog as $item)
                                            ...
                                            @else
                                                <option value="{{ $item->id }}" {{ $product->catalog_id == $item->id ? 'selected' : '' }}>{{ $item->title['ru'] }}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mb-4">
                                    ...
                                    <select class="form-select" id="type" name="type">
                                        ...
                                        <option value="med" {{ $product->type == 'med' ? 'selected' : '' }}>Медицинская</option>
                                        <option value="water" {{ $product->type == 'water' ? 'selected' : '' }}>Для напиток</option>
                                    </select>
                                </div>
                                <div class="mb-4">
                                    <label for="email">Vendor code</label>
                                    <input type="text" class="form-control" name="vendor_code" placeholder="vendor code..." value="{{ old('vendor_code') ? old('vendor_code') : $product->vendor_code }}">
                                </div>
                                <div class="mb-4">
                                    <label for="email">Order</label>
                                    <input type="text" class="form-control" name="order" placeholder="order..." value="{{ old('order')?? $product->order }}">
                                </div>
                            </div>
                        </div>
                        <div class="row mb-4">
