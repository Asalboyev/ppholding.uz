...
@section('content')
    ...
    <div class="headcrumbs d-flex mb-3">
        <a href="{{ route('news.index') }}" class="d-flex text-uppercase me-2" style="opacity:25%">news</a> >> <a class="d-flex text-uppercase ms-2">edit</a>
    </div>
                                    ...
                                    <div class="row mb-4">
                                        <div class="col-lg-6 col-sm-6">
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <input type="text" class="form-control" name="title[{{ $language->small }}]" placeholder="title" value="{{ old('title.'.$language->small) ? old('title.'.$language->small) : $post->title[$language->small] }}">
                                            </div>
                                            <div class="my-4">
                                                <label for="textarea">Description</label>
                                                <textarea class="form-control ckeditor" placeholder="Enter your description..." id="textarea" rows="4" name="desc[{{ $language->small }}]">{{ old('desc.'.$language->small) ? old('desc.'.$language->small) : $post->desc[$language->small] }}</textarea>
                                            </div>
                                            <!-- End of Form -->
                                        </div>
                                    </div>
